...
%%
%% @see with_api/4
-spec with_api(permission(), rb_hex_core:config(), fun((rb_hex_core:config()) -> Result)) ->
...
%% Example:
%% ```
%% rb_hex_cli_auth:with_api(write, Config, fun(C) ->
%%     rb_hex_api_release:publish(C, Tarball)
%% end, [{optional, false}, {auth_inline, true}]).
%% '''
-spec with_api(
    permission(),
    rb_hex_core:config(),
    fun((rb_hex_core:config()) -> Result),
    opts()
) ->
    Result | {error, auth_error()}
when
...
%%
%% @see with_repo/3
-spec with_repo(rb_hex_core:config(), fun((rb_hex_core:config()) -> Result)) ->
    Result | {error, auth_error()}
when
    Result :: term().
...
%% Example:
%% ```
%% rb_hex_cli_auth:with_repo(Config, fun(C) ->
%%     rb_hex_repo:get_tarball(C, <<"ecto">>, <<"3.0.0">>)
%% end).
%% '''
-spec with_repo(rb_hex_core:config(), fun((rb_hex_core:config()) -> Result), opts()) ->
    Result | {error, auth_error()}
when
    Result :: term().
...
-spec repo_name(rb_hex_core:config()) -> binary().
...
-spec device_auth(rb_hex_core:config(), binary(), opts()) ->
...
device_auth(Config, Scope, Opts) ->
    ...
    case rb_hex_api_oauth:device_auth_flow(Config, ClientId, Scope, PromptUser, FlowOpts) of
...
%% @private
-spec resolve_api_auth(permission(), rb_hex_core:config()) ->
...
-spec resolve_repo_auth(rb_hex_core:config()) ->
...
exchange_for_oauth_token(RepoName, Config, AuthKey, Scope) ->
    ...
    case rb_hex_api_oauth:client_credentials_token(ExchangeConfig, ClientId, AuthKey, Scope) of
...
->
    ClientId = call_callback(Config, get_client_id, []),
    case rb_hex_api_oauth:refresh_token(Config, ClientId, RefreshToken) of
...
%% @private
-spec detect_auth_error(rb_hex_http:headers()) -> otp_required | invalid_totp | token_expired | none.
