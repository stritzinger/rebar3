...
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%%
%% > rb_hex_tarball:unpack(Tarball, none).
%% {ok,#{outer_checksum => <<...>>,
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%%
...
%% '''
-spec unpack
    (tarball() | {file, filename()}, memory, rb_hex_core:config()) ->
        {ok, #{
            outer_checksum => checksum(),
        ...
        | {error, term()};
    (tarball() | {file, filename()}, none, rb_hex_core:config()) ->
        {ok, #{
            outer_checksum => checksum(),
            inner_checksum => checksum(),
            metadata => metadata()
        ...
        | {error, term()};
    (tarball() | {file, filename()}, filename(), rb_hex_core:config()) ->
        {ok, #{
            outer_checksum => checksum(),
            inner_checksum => checksum(),
            metadata => metadata()
...
unpack(Input, memory, Config) ->
    case check_input_size(Input, Config) of
        true ->
            ...
            case rb_hex_erl_tar:extract(Source, [memory]) of
                {ok, []} ->
                    {error, {tarball, empty}};
                {ok, FileList} ->
...
unpack(Input, Output, Config) ->
    case check_input_size(Input, Config) of
        true ->
            ...
            try
                case rb_hex_erl_tar:extract(Source, [{cwd, TmpDir}]) of
                    ok ->
...
-spec unpack_docs
    ...
    (tarball() | {file, filename()}, filename(), rb_hex_core:config()) -> ok | {error, term()}.
...
unpack_docs(Input, Output) ->
    unpack_docs(Input, Output, rb_hex_core:default_config()).
...
unpack_contents(Contents, Output, MaxSize) ->
    ...
    case rb_hex_erl_tar:extract(Source, [{max_size, MaxSize} | Opts]) of
        {error, too_big} ->
...
%% @private
%% Streams the metadata.config binary through rb_safe_erl_term:tokens/2 in
...
%% @private
feed_metadata(Encoding, Cont, Chars, Binary, IncTail, Acc) ->
    case rb_safe_erl_term:tokens(Cont, Chars) of
        {more, NewCont} ->
        ...
        {done, {error, {_, rb_safe_erl_term, Reason}, _}, _} ->
...
flush_metadata_eof([], Acc) ->
    finalize_metadata(Acc);
flush_metadata_eof(Cont, Acc) ->
    case rb_safe_erl_term:tokens(Cont, eof) of
        {done, {eof, _}, _} ->
            finalize_metadata(Acc);
        {done, {ok, _Tokens, _}, _} ->
...
%% @private
%% Streams the metadata.config binary through rb_safe_erl_term:token/2 one token
...
%% @private
feed_metadata_streaming(Encoding, Cont, Chars, Binary, IncTail, Acc, Fields, State) ->
    case rb_safe_erl_term:token(Cont, Chars) of
        {more, NewCont} ->
...
flush_metadata_streaming_eof([], Acc, _Fields, State) ->
    finalize_metadata_streaming(Acc, State);
flush_metadata_streaming_eof(Cont, Acc, Fields, State) ->
    case rb_safe_erl_term:token(Cont, eof) of
        {done, {ok, Token, _}, _} ->
            case advance_metadata_state(State, Acc, Fields, Token) of
                {next, NewState, NewAcc} ->
...
unpack_tarball(Source, memory, MaxSize) ->
    case rb_hex_erl_tar:extract(Source, [memory, compressed, {max_size, MaxSize}]) of
        {error, too_big} ->
...
unpack_tarball(Source, Output, MaxSize) ->
    filelib:ensure_dir(filename:join(Output, "*")),
    case rb_hex_erl_tar:extract(Source, [{cwd, Output}, compressed, {max_size, MaxSize}]) of
        ok ->
            [
...
add_file(Tar, {Filename, AbsFilename}) when is_list(Filename), is_list(AbsFilename) ->
    ...
    case FileInfo#file_info.type of
        ...
        _ ->
            ...
            ok = rb_hex_erl_tar:add(Tar, Contents, Filename, [{mode, Mode} | tar_opts()])
    end.
