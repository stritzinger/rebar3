...
%% @end
-spec device_authorization(rb_hex_core:config(), binary(), binary()) -> rb_hex_api:response().
...
%%
%% ```
%% 1> Config = rb_hex_core:default_config().
%% 2> rb_hex_api_oauth:device_authorization(Config, <<"cli">>, <<"api:write">>).
%% {ok,{200, ..., #{
%%     <<"device_code">> => <<"...">>,
%%     <<"user_code">> => <<"ABCD-1234">>,
...
%% }}}
%%
%% 3> rb_hex_api_oauth:device_authorization(Config, <<"cli">>, <<"api:write">>, [{name, <<"MyMachine">>}]).
%% '''
%% @end
-spec device_authorization(rb_hex_core:config(), binary(), binary(), proplists:proplist()) ->
    rb_hex_api:response().
...
%%    end.
%% 3> rb_hex_api_oauth:device_auth_flow(Config, <<"cli">>, <<"api:write">>, PromptUser).
%% {ok, #{
%%     access_token => <<"...">>,
%%     refresh_token => <<"...">>,
...
%% ```
%% 1> Config = rb_hex_core:default_config().
%% 2> rb_hex_api_oauth:poll_device_token(Config, <<"cli">>, DeviceCode).
%% {ok, {200, _, #{
%%     <<"access_token">> => <<"...">>,
%%     <<"refresh_token">> => <<"...">>,
%%     <<"token_type">> => <<"Bearer">>,
...
%% '''
%% @end
-spec poll_device_token(rb_hex_core:config(), binary(), binary()) -> rb_hex_api:response().
...
%% ```
%% 1> Config = rb_hex_core:default_config().
%% 2> rb_hex_api_oauth:refresh_token(Config, <<"cli">>, RefreshToken).
%% {ok, {200, _, #{
%%     <<"access_token">> => <<"...">>,
%%     <<"refresh_token">> => <<"...">>,
%%     <<"token_type">> => <<"Bearer">>,
...
%% '''
%% @end
-spec refresh_token(rb_hex_core:config(), binary(), binary()) -> rb_hex_api:response().
...
%% @end
-spec client_credentials_token(rb_hex_core:config(), binary(), binary(), binary()) ->
    rb_hex_api:response().
...
%% ```
%% 1> Config = rb_hex_core:default_config().
%% 2> rb_hex_api_oauth:client_credentials_token(Config, <<"cli">>, ApiKey, <<"api">>).
%% {ok, {200, _, #{
%%     <<"access_token">> => <<"...">>,
%%     <<"token_type">> => <<"bearer">>,
%%     <<"expires_in">> => 1800,
...
%% }}}
%%
%% 3> rb_hex_api_oauth:client_credentials_token(Config, <<"cli">>, ApiKey, <<"api">>, [{name, <<"MyMachine">>}]).
%% '''
%% @end
-spec client_credentials_token(
    rb_hex_core:config(), binary(), binary(), binary(), proplists:proplist()
) -> rb_hex_api:response().
...
%% ```
%% 1> Config = rb_hex_core:default_config().
%% 2> rb_hex_api_oauth:revoke_token(Config, <<"cli">>, Token).
%% {ok, {200, ..., nil}}
%% '''
%% @end
-spec revoke_token(rb_hex_core:config(), binary(), binary()) -> rb_hex_api:response().
